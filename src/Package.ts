...
import type { WPMMconfigPkg, WPMMpaths } from "./types.js";
import { installComposer, installNpmPackages } from "./utils/commands.js";
import { getDownloadUrl } from "./utils/data.js";
import { downloadFile, extractZip, renameFolder } from "./utils/fs.js";
...
export class Package {
	 ...
	 */
	constructor(pkgConfig: WPMMconfigPkg, packageType: string, paths: WPMMpaths) {
	...
	async execDownload(filename: string, downloadUrl: string): Promise<string> {
		...
		console.log(
			`⬇ Downloading ${zipFileName} to ${zipFilePath} (source ${downloadUrl})`,
		);
	...
	async downloadPackage(packageUrl: string): Promise<void> {
		try {
			...
			if (fs.existsSync(pkgFolder)) {
				console.log(
					`i destination folder ${pkgFolder} already exists. Skipping download.`,
				);
				return;
			}

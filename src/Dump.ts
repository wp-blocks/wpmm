...
import type { WPMMconfig, WPMMconfigPkg, WPMMpaths } from "./types.js";
import { isValidWPConfig, parseWpConfig } from "./utils/parsers.js";
import { getCurrentWpInfo, getWpConfigContent } from "./utils/wordpress.js";
...
export class Dump {
	...
	async init(): Promise<WPMMconfig> {
		...
		console.log(`🔍 Scanning ${this.baseFolder}`);
		...
		for (const item of items) {
			...
			if (isDirectory) {
				...
				if (fs.existsSync(stylePath)) {
					...
					if (version) {
						console.log(`i Found ${item} version ${version}`);
	 ...
	 */
	extractVersionFromPHPFile(filePath: fs.PathOrFileDescriptor): string | null {
		const content = fs.readFileSync(filePath, "utf8");
		const match = /Version:\s*([\d.]+)/i.exec(content);
		return match ? match[1] : null;
	}

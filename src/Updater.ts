...
import type { UpdateObject, WPMMconfig } from "./types.js";
...
export class Updater {
	...
	async updateThemes(): Promise<void> {
		try {
			...
			throw new Error(`Error updating Themes: ${(error as Error).message}`);
		}
	}
	...
	async updateWordPress() {
		try {
			...
			throw new Error(`Error updating Wordpress: ${(error as Error).message}`);
		}
	}

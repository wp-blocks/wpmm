...
import { getWordPressDownloadUrl } from './utils/data'
import type { WordpressPkg, WPMMpaths } from './types'
    ...
    async installWordPress(
        ...
        try {
            ...
            if (fs.existsSync(destinationPath)) {
                console.log(
                    '🔄️ WordPress folder already exists. Skipping download.'
                )
            } else {
                // Download WordPress
                return await this.execDownload(
                    ...
                    renameFolder(
                        path.join(this.tempDir, 'wordpress'),
                        destinationPath
                    )
                    console.log(
            ...
            console.error(
                '🔴 Error downloading or installing WordPress:',
                error
            )
        }
    }
    ...
    async setupWordPressConfig(): Promise<void> {
        ...
        try {
            if (fs.existsSync(configPath)) {
                console.log(
                    '🆗 WordPress configuration already set up. updating...'
                )
            } else {

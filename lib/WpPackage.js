...
const { replaceDbConstant, replaceDbConstantBool, replaceEmptySalts } = require('./utils/wpConfig.js');
const { getWordPressDownloadUrl } = require('./utils/index.js');
...
class WpPackage extends Package {
  ...
  async installWordPress (version, language) {
    ...
    try {
      const destinationPath = path.join(this.rootFolder, this.pkgInfo.name);
  ...
  async setupWordPressConfig () {
    ...
    try {
      if (fs.existsSync(configPath)) {
        ...
        const sampleConfigPath = path.join(this.destFolder, 'wp-config-sample.php');

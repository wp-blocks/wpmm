...
class Package {
        ...
        return await new Promise((resolve, reject) => {
          exec(`git clone ${packageUrl} ${targetDirectory}/${packageName}`, (error, stdout, stderr) => {
            if (error) {
              ...
              resolve(path.join(targetDirectory, packageName));
            }
          });
        });
      } else {
   ...
   */
  async installPackage (packageName, packageVersion, packageType, repoUrl, typeOfPackage) {
    const packageUrl = repoUrl || getDownloadUrl(packageName, packageVersion, packageType);
    const targetDirectory = this.destFolder;

    const result = await this.downloadPackage(packageUrl, packageName, targetDirectory);
    if (typeof result === 'string') {
      this.installRepository(result).then(() => {
        console.log(`🆗 ${packageType} ${packageName} installed successfully.`);
      });
    } else {
      console.log(`Downloading the ${typeOfPackage} from Wordpress...`);
    }
  }

  async installRepository (repoPath) {
    try {
      ...
      console.log('Neither *.json exists.');
    }

    async function runNpm () {
      console.log('Found package.json');
      await exec('npm install', { cwd: repoPath });
      await exec('npm run build', { cwd: repoPath });
    }
    async function runComposer () {
      console.log('Found composer.json');
      await exec('composer install', { cwd: repoPath });
...
class WordPressPackage extends Package {
  ...
  async installWordPress (version, language) {
    ...
    try {
      const destinationPath = path.join(this.rootFolder, 'wp-folder');
      ...
      if (fs.existsSync(destinationPath)) {
        ...
        if (!fs.existsSync(path.join(destinationPath))) {
          fs.mkdirSync(destinationPath);
        }
        fs.renameSync(path.join(this.tempDir, 'wordpress'), path.join(this.baseFolder));
        console.log(`🆗 WordPress installed successfully in ${this.baseFolder}`);
      }
    } catch (error) {
...
class PluginPackage extends Package {
   ...
   */
  async install () {
    const { name, version, repo = null } = this.config;
    await this.installPackage(name, version, 'plugin', repo);
...
class ThemePackage extends Package {
  ...
  async install () {
    const { name, version, repo = null } = this.config;
    await this.installPackage(this.config.name, this.config.version, 'theme', repo);

...
class Package {
  ...
  async downloadPackage (packageUrl, packageName, targetDirectory) {
    try {
      ...
      if (packageUrl.split('.').pop() === 'git') {
        extractedPath = await this.cloneRepo(packageUrl, packageName, targetDirectory);
      } else {
        // Download the package
        extractedPath = await this.execDownload(packageName, packageUrl);
      }
      ...
      if (packageUrl) {
        const destinationFolder = path.join(packageUrl, packageName);
  ...
  async installPackage (config, packageType) {
    ...
    await this.downloadPackage(packageUrl, name, this.destFolder);
  }
}
  ...
  async installWordPress (version, language) {
    ...
    try {
      ...
      if (fs.existsSync(destinationPath)) {
        ...
        // Download WordPress
        await this.execDownload(`wordpress-${version}`, downloadUrl + '.zip');
...
class PluginPackage extends Package {
   ...
   */
  async install () {
    await this.installPackage(this.config, 'plugin');
...
class ThemePackage extends Package {
   ...
   */
  async install () {
    await this.installPackage(this.config, 'theme');

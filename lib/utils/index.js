...
function getConfig (args) {
  ...
  if (! args?.template) {
    const templatePath = 'wp-package.json';
    ...
    if (!fs.existsSync(templatePath)) {
      ...
      /**
       * Read the user-defined configuration object from the template file and add it to the config
    ...
    get(args.template)
      .then(response => {
        const gitConfig = response.data;
        config = {...defaultConfig, ...gitConfig};
      })
      .catch(error => {

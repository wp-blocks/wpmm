 ...
 * @return {Promise<object>} Resolves with the fetched data if successful, otherwise logs the error to the console.
 */
 ...
 *
 * @returns {object} The Last available WordPress version from WordPress.org
 * @throws {Error} If the request to WordPress.org fails
 */
async function getLastWp() {
  try {
    const response = await axiosFetch('https://api.wordpress.org/core/version-check/1.7/');
